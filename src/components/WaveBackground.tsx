...
export default function WaveBackground() {
  ...
  // Limit performance-heavy elements on mobile to guarantee smooth scrolling
  const rayCount = isMobile ? 2 : 8;
  const rippleCount = isMobile ? 3 : 15;
      ...
      <div className="absolute inset-[-15%] opacity-60 select-none" style={{ animation: 'water-drift 25s ease-in-out infinite' }}>
        ...
        <div className="absolute inset-0 z-0">
          {[...Array(rayCount)].map((_, i) => (
            <div
              key={`ray-${i}`}
              className="absolute top-[-30%] h-[180%] w-[12vw] bg-gradient-to-b from-[var(--color-primary)]/15 via-[var(--color-primary)]/5 to-transparent blur-[50px] lg:blur-[80px]"
              style={{
                left: `${5 + i * (isMobile ? 40 : 15)}%`,
        ...
        <div className="absolute inset-0 mix-blend-screen overflow-hidden">
          {[...Array(rippleCount)].map((_, i) => (
            <div
              key={`ripple-${i}`}
              className="absolute rounded-full bg-[var(--color-primary)]/10 blur-[60px] lg:blur-[100px]"
              style={{
                width: isMobile ? '200px' : `${300 + Math.random() * 500}px`,
                height: isMobile ? '200px' : `${300 + Math.random() * 500}px`,
                top: `${(i * (100 / rippleCount)) + (isMobile ? 10 : Math.random() * 20)}%`,
                left: `${(i * (100 / rippleCount)) + (isMobile ? 10 : Math.random() * 20)}%`,
      ...
      <div className="absolute inset-0 z-30 overflow-hidden pointer-events-none">
        ...
        {[...Array(bubbleCount)].map((_, i) => (
          <div
            ...
            style={{
              left: `${(i * (100 / bubbleCount)) + Math.random() * 5}%`,
